...
import { calcSave, chargeHours, mergeSave } from "./save.ts";
...
test("tempo de recarga 20% → 80%", () => {
  assert.ok(Math.abs(chargeHours(11) - 36 / 9.9) < 1e-9);
  assert.ok(chargeHours(22) < chargeHours(2.3));
});
