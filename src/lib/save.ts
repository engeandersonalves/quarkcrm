...
export const DEFAULT_SAVE_CONDITIONS = [
  "A potência do carregador exige alimentação compatível no ponto de conexão. A velocidade real de recarga também depende do veículo: cada modelo aceita uma potência máxima.",
...
export const REF_BATTERY_KWH = 60;
/** Tomada comum de casa (10 A · 220 V). */
export const COMMON_SOCKET_KW = 2.3;

/** Horas para recarregar de 20% a 80% (60% da bateria), com ~90% de eficiência. */
export function chargeHours(kw: number, batteryKwh = REF_BATTERY_KWH) {
  return (n(batteryKwh) * 0.6) / (Math.max(0.1, n(kw)) * 0.9);
}
